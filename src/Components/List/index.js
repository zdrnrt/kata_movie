...
export default function List(props) {
	...
	if (status.load) {
		...
		if (!!status.data && status.data?.total_results > 0) {
			...
			pagination = <Pagination style={{maxWidth: 'fit-content', margin: [0, 'auto']}} hideOnSinglePage={true} showSizeChanger={false} showQuickJumper={false} current={status.data.page} total={status.data.total_pages} pageSize={5} pageSizeOptions={[5, 10]} onChange={(page) => props.listener.page(page)} />
		}
	...
	return (
		<div style={{padding: '20px 0'}}>
			<Input value={request.query} placeholder='Type to search...' size='large' onChange={(event) => props.listener.query(event.target.value)}/>
			{!!result && <Row gutter={[36, 36]} align="stretch" style={{padding: '34px 0'}}>
				{result}
			</Row>}
			{pagination}
		</div>
